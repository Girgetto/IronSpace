...
Game.prototype.level = function (ctx) {
  if (this.frame === 1) {
    this.goal = [100, 100];
    this.planet = [500, 500, 50, 10 ** 8];
  }
  if (this.frame === 2) {
    this.goal = [900, 500];
    this.planet = [800, 500, 50, 10 ** 8];
    this.planet2 = [500, 100, 70, 10 ** 8];
    this.firstClick = false;
  }
  if (this.frame === 3) {
    this.goal = [1000, 500];
    this.planet = [800, 500, 80, 10 ** 8];
    this.planet2 = [500, 300, 70, 10 ** 8];
  }
  if (this.frame === 4) {
    this.goal = [ctx.canvas.width / 2, ctx.canvas.height / 2];
    this.planet = [600, 200, 80, 10 ** 8];
    this.planet2 = [400, 350, 60, 10 ** 8];
    this.planet3 = [600, 600, 70, 10 ** 8];
  }
  this.frame++;
  return true;
};
